...
export default function AppLayout({ title, action, children }) {
  return (
    <div className="App flex min-h-screen bg-white">
      <Sidebar />
      {/* Main content — flex-1 so it fills whatever space Sidebar leaves */}
      <div className="flex-1 flex flex-col min-w-0 pt-[56px] md:pt-0">
        <header className="h-[72px] flex items-center justify-between px-6 md:px-8 border-b border-[#E5E7EB] bg-white shrink-0">
          <h1
        ...
        <main className="flex-1 p-6 md:p-8 bg-white" data-testid="page-main">
          {children}
        </main>
      </div>
    </div>
...
export function StatusBadge({ status }) {
  ...
  return (
    <span data-testid={`status-${status}`} className={map[status] || "q-badge q-badge-draft"}>
      {status}
    </span>
  );
}
